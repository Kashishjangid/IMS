...
const Navbar3 = () => {
  ...
  const Menus = [
    ...
    { title: "Logout", icon: <TbLogout />, path: "/dashboard"},
  ];
    ...
    <>
      <div className='max-md:fixed top-0 h-20 z-[99] w-full'>
        <div className="w-fit  md:block hidden h-screen     bg-[#184892] ">
          ...
          <div className={` px-5 py-8    ${open ? 'w-60' : 'w-20'} duration-300 relative z-40`}>
            ...
            <div className={`flex items-center rounded-md mt-6 gap-2 bg-white  py-2 ${open ? 'px-4' : 'px-2.5'} `}>
              <FaSearch />
              <input type='text' className={`outline-none ${open ? 'block' : 'hidden'} overflow-hidden`} placeholder='Search'></input>
            </div>

            <ul className='pt-4 '>
              {Menus.map((menu, index) => (
                <>
                  <Link key={index} href={menu.path} className='flex hover:bg-white  *:hover:text-[#184892] rounded-md cursor-pointer'>
                    <li className={`text-white    flex gap-2  text-xl items-center  ${open ? 'p-2' : 'p-1.5'} `}>
        ...
        <div className={`border bg-[#184892] h-screen   text-white z-20  border-none  md:hidden ${mobileToggle ? 'w-0' : 'w-[60%]'} duration-300 overflow-hidden  `}>
          <ul className=' mt-6 '>
            ...
            {Menus.map((menu, index) => (
              <Link href={menu.path} className='*:px-4 *:py-2 ' onClick={() => setMobileToggle(!mobileToggle)} key={index+1}>
